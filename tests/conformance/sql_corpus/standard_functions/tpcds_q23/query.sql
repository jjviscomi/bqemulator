WITH frequent_ss_items AS (
  SELECT
    ...
    d_date AS solddate,
    COUNT(*) AS cnt
  FROM `${DATASET}.store_sales`, `${DATASET}.date_dim`, `${DATASET}.item`
...
max_store_sales AS (
  ...
  FROM (
    ...
    FROM `${DATASET}.store_sales`, `${DATASET}.customer`, `${DATASET}.date_dim`
...
best_ss_customer AS (
  ...
  FROM `${DATASET}.store_sales`, `${DATASET}.customer`
  WHERE ss_customer_sk = c_customer_sk
  GROUP BY c_customer_sk
