-- TPC-DS Q98 setup — store_sales ⋈ item ⋈ date_dim. Per-class item-revenue
-- ratio via SUM(ss_ext_sales_price) OVER (PARTITION BY i_class). Spec params:
-- i_category IN ('Sports','Books','Home'); d_date between 1999-02-22 and
-- 1999-02-22 + 30 days. Sister-query to Q12 (web_sales) and Q20 (catalog_sales).
...
INSERT INTO `${DATASET}.store_sales` VALUES
  ...
  (2451232, 1, NUMERIC  "90.00"),
  (2451240, 1, NUMERIC  "70.00"),
  (2451250, 2, NUMERIC "220.00"),
  -- class=fiction: item 3 + item 4
  (2451240, 3, NUMERIC  "80.00"),
  (2451250, 4, NUMERIC "120.00"),
  -- class=lighting: item 5
  (2451262, 5, NUMERIC "170.00"),
