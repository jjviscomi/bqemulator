WITH all_sales AS (
  ...
  FROM (
    SELECT
      ...
      cs_ext_sales_price - COALESCE(cr_return_amount, NUMERIC "0") AS sales_amt
    ...
    LEFT JOIN `${DATASET}.catalog_returns` AS catalog_returns
    ...
    SELECT
      ...
      ss_ext_sales_price - COALESCE(sr_return_amt, NUMERIC "0") AS sales_amt
    FROM `${DATASET}.store_sales` AS store_sales
    JOIN `${DATASET}.item` AS item
      ON i_item_sk = ss_item_sk
    JOIN `${DATASET}.date_dim` AS date_dim
      ON d_date_sk = ss_sold_date_sk
    LEFT JOIN `${DATASET}.store_returns` AS store_returns
    ...
    SELECT
      ...
      ws_ext_sales_price - COALESCE(wr_return_amt, NUMERIC "0") AS sales_amt
    FROM `${DATASET}.web_sales` AS web_sales
    JOIN `${DATASET}.item` AS item
      ON i_item_sk = ws_item_sk
    JOIN `${DATASET}.date_dim` AS date_dim
      ON d_date_sk = ws_sold_date_sk
    LEFT JOIN `${DATASET}.web_returns` AS web_returns
